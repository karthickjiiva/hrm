...
class WFHLeaveController extends ApiBaseController
{
    protected $model = WfhLeave::class;
    ...
    protected function modifyIndex($query)
{
    ...
    if ($request->has('status') && $request->status != "all") {
        ...
        $query = $query->where('leaves_wfh.status', $leaveStatus);
    }

    if ($loggedUser->ability('admin', 'leaves_view')) {
        $query = $this->applyVisibility($query, 'leaves_wfh');
        ...
        $query = $query->where('leaves_wfh.user_id', $loggedUser->id);
    }

    // leaveType Filters
    if ($request->has('leave_type_id') && $request->leave_type_id != "") {
        $leaveTypeId = $this->getIdFromHash($request->leave_type_id);
        $query = $query->where('leaves_wfh.leave_type_id', $leaveTypeId);
    }

    return $query;
} 

    public function storing(WfhLeave $leave)
    {
        $loggedUser = user();
        $request = request();
    ...
    public function stored(WfhLeave $leave)
    {
        $loggedUser = user();
  ...
  public function updateRaw($id)
{
    try {
        ...
        $leave = \App\Models\WfhLeave::findOrFail($id);

        $data = request()->all();

        // decode user
        $userId = $this->getIdFromHash($data['user_id']);
        ...
        $leave->update([
            ...
            'is_half_day' => $entry['is_half_day'] ?? 0,
            'half_day_type' => ($entry['is_half_day'] ?? 0)
                ? ($entry['half_day_type'] ?? null)
                : null,
    ...
    public function updating(WfhLeave $leave)
    {
        $loggedUser = user();
        $request = request();
    ...
    public function destroying(WfhLeave $leave)
    {
        $loggedUser = user();
