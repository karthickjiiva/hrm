    ...
    public function array(array $users)
    {
        DB::transaction(function () use ($users) {
            ...
            foreach ($users as $user) {
                ...
                $name = trim($user['name']);
                $nameCount = StaffMember::withoutGlobalScope('type')->where('name', $name)->where('user_type', $userType)->count();
                if ($nameCount > 0) {
                    throw new ApiException('User ' . $name . ' Already Exists');
                }

                $email = trim($user['email']);
                $emailCount = StaffMember::withoutGlobalScope('type')->where('email', $email)->where('user_type', $userType)->count();
                if ($emailCount > 0) {
                    throw new ApiException('Email ' . $email . ' Already Exists');
                }

                $phone = trim($user['phone']);
                $phoneCount = StaffMember::withoutGlobalScope('type')->where('phone', $phone)->where('user_type', $userType)->count();
                if ($phoneCount > 0) {
