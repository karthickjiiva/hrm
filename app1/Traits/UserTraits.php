...
trait UserTraits
{
    ...
    public function modifyIndex($query)
    {
        ...
        if (!$request->has('fields')) {
            $query = $query->with(['location:id,name', 'designation:id,name', 'department:id,name']);
        }
    ...
    public function stored($user)
    {
        ...
        Payrolls::updateEmployeeSalary($user->id, $user->basic_salary, $user->monthly_amount, $user->annual_amount, $user->annual_ctc, $user->ctc_value, $user->net_salary, $user->special_allowances, $request->salary_components, $request->salary_group_id);
    }
