...
@section('content')
    ...
    @php
        ...
        $specialAllowance = $payroll->basic_salary - $totalMonthlyEarning;

        //gross earnings sum
        $grossEarnings = round($payroll->basic_salary + $specialAllowance + $totalMonthlyEarning, 2);

        //earning components
        $earningsComponents = collect($payroll->payrollComponents)
            ->whereIn('type', ['earnings', 'custom_earning', 'additional_earning'])
            ->values();
        ...
        <div style="width: 49%; float: left; margin-right: 10px;">
            <table style="width: 100%; margin-top: 10px; margin-bottom: 10px;">
                ...
                <tr>
                    ...
                    <td class="table-text" style="text-align: right;">{{ number_format($payroll->basic_salary, 2) }}</td>
                </tr>
                    ...
                    @foreach ($earningsComponents as $component)
                        <tr>
                            ...
                            <td class="table-text" style="text-align: right;">
                                {{ number_format($component->monthly_value ?? 0, 2) }}</td>
                        </tr>
                    @endforeach
                @endif
                <tr>
                    ...
                    <td class="table-text" style="text-align: right;">{{ number_format($specialAllowance ?? 0, 2) }}</td>
                </tr>
            </table>
        </div>
                    ...
                    @foreach ($components as $component)
                        <tr>
                            ...
                            <td class="table-text" style="color: black;text-align: right">
                                {{ number_format($component['monthly_value'], 2) }}
                            </td>
                        </tr>
                    @endforeach
                @endforeach
        ...
        <div style="width: 49%; float: left;">
            <table style="width: 100%; margin-bottom: 10px;">
                <tr>
                    ...
                    <th class="table-text" style="text-align: right;">{{ $grossEarnings }}</th>
                </tr>
            </table>
        </div>
            ...
            <table style="width: 100%;">
                <tr>
                    <th class="table-text" style="text-align: left;width: 50%">
                        {{ $payrollTranslation['total_deductions'] }}</th>
                    <th class="table-text" style="text-align: right;width: 50%">{{ $totalMonthlyDeductions }}</th>
                </tr>
            </table>
        </div>
    </div>
    <div style="width: 100%; margin-top: 10px;">
        <table style="width: 100%; margin-top: 10px; margin-bottom: 10px;">
            ...
            <tr>
                ...
                <td class="table-text" style="text-align: right;">
                    {{ number_format(optional($payroll)->expense_amount ?? 0, 2) }}</td>
            </tr>
        </table>
    </div>
    ...
    <div style="width: 100%; margin-top: 10px;text-align:center">
        <span style="font-weight:bold;font-size:25px">{{ $payrollTranslation['net_salary'] . ':' }}</span>&nbsp;&nbsp;<span
            style="font-size:25px">{{ $payroll->net_salary = round($payroll->net_salary, 2) }}</span>
    </div>
