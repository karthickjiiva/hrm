...
export default {
    ...
    setup(props, { emit }) {
        ...
        const downloadPdf = () => {
            ...
            if (isValid) {
                if (props.url === "account-csv") {
                    docType = "text/csv";
                    type = "csv";
                } else if (props.url === "account-xlsx") {
                    docType =
                ...
                return axiosPdf
                    ...
                    .then((response) => {
                        ...
                        if (props.isPrint) {
                            ...
                            link.setAttribute("download", `${props.fileName}.${type}`);
        ...
        const validateForm = () => {
            ...
            if (props.payload) {
                if (!props.payload.xid) {
                    rule.value["account_id"] = {
                        message: "account_id field is required",
                    };
                    isValid = false;
                }
                if (!props.payload.date || props.payload.date.length == 0) {
                    rule.value["date"] = { message: "date range field is required" };
                    isValid = false;
                }
            }
            return isValid;
