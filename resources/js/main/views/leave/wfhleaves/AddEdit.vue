...
export default defineComponent({
    ...
    const payload = {
        ...
        leave_type_id: 1,
        entries: selectedDates.value.map(entry => ({
            date: entry.date,
    ...
    try {
        ...
        emit("addEditSuccess", res.data.data || res.data);

    } catch (err) {
        console.log("REAL ERROR", err.response);
    }
    ...
    (newVal) => {
        ...
        if (props.addEditType === "add") {
            selectedDates.value = []; // keep add working ✅
        } else {
            // EDIT MODE

            // set user
            props.formData.user_id = props.data.x_user_id;

            // set selected dates (IMPORTANT)
            selectedDates.value = [
                {
                    date: dayjs(props.data.start_date).format("YYYY-MM-DD"),
                    is_half_day: props.data.is_half_day,
                    half_day_type: props.data.half_day_type,
                },
            ];
        }
    }
