<template>
    <AdminPageHeader>
        <template #header>
            <a-page-header title="Form X - Leave & Social Security Register" class="p-0" />
        </template>
        <template #breadcrumb>
            <a-breadcrumb separator="-" style="font-size: 12px">
                ...
                <a-breadcrumb-item>Form X (Leaves)</a-breadcrumb-item>
            </a-breadcrumb>
        </template>
    </AdminPageHeader>
    ...
    <admin-page-table-content>
        ...
        >
            ...
            Download
                        ...
                        <!-- <a-popconfirm title="Delete this report?" @confirm="deleteReport(record.id)">
...
import { DownloadOutlined, FileExcelOutlined, ExclamationCircleOutlined, DeleteOutlined } from "@ant-design/icons-vue";
import { Modal, message } from "ant-design-vue";
import axios from "axios";

export default {
    components: { DownloadOutlined, FileExcelOutlined, DeleteOutlined, AdminPageHeader },
    setup() {
        ...
        const columns = [
            { title: "Report Period", dataIndex: "month" },
        ...
        const fetchHistory = async () => {
            loading.value = true;
            try {
                const res = await axios.get(`/api/v1/reports/history/form_x`, {
        ...
        const confirmGenerate = () => {
            Modal.confirm({
                title: `Generate Form X for ${getMonthName(filters.month)} ${filters.year}?`,
                icon: createVNode(ExclamationCircleOutlined),
                content: "This will compile the Leave Register (Opening, Earned, Availed, Closing) for the selected period.",
                okText: 'Generate',
                async onOk() {
        ...
        const generateReport = async () => {
            ...
            try {
                const res = await axios.post(`/api/v1/reports/generate/form_x`, filters);
        ...
        return { 
            filters, yearOptions, monthOptions, history, loading, 
            generating, columns, pagination, handleTableChange, 
            confirmGenerate, downloadReport, getMonthName, deleteReport 
        };
    }
};
</script>